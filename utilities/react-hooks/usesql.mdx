...
Hook which executes a query on a local SQL database and returns the [AsyncState](/utilities/react-hooks/usesql#asyncstate) corresponding to the execution of the query. The last value will be kept between command runs.
...
Returns an object with the [AsyncState](/utilities/react-hooks/usesql#asyncstate) corresponding to the execution of the function as well as a couple of methods to manipulate it.

* `data`, `error`, `isLoading` \- see [AsyncState](/utilities/react-hooks/usesql#asyncstate).
...
* `mutate` is a method to wrap an asynchronous update and gives some control over how the `useSQL`'s data should be updated while the update is going through. By default, the data will be revalidated (eg. the function will be called again) after the update is done. See [Mutation and Optimistic Updates](/utilities/react-hooks/usesql#mutation-and-optimistic-updates) for more information.
...
const Demo = () => {
  const { isLoading, data, permissionView } = useSQL(NOTES_DB, notesQuery);
