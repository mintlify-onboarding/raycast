...
Hook that executes a command and returns the [AsyncState](/utilities/react-hooks/useexec#asyncstate) corresponding to the execution of the command. The last value will be kept between command runs.
...
function useExec<t, u>(
  ...
  options?: {
    ...
    input?: string | Buffer;
    timeout?: number;
    parseOutput?: ParseExecOutputHandler;
    initialData?: U;
...
* `options.parseOutput` is a function that accepts the output of the child process as an argument and returns the data the hooks will return - see [ParseExecOutputHandler](/utilities/react-hooks/useexec#parseexecoutputhandler). By default, the hook will return `stdout`.

Including the [useCachedPromise](/utilities/react-hooks/usecachedpromise)'s options:
...
Returns an object with the [AsyncState](/utilities/react-hooks/useexec#asyncstate) corresponding to the execution of the command as well as a couple of methods to manipulate it.

* `data`, `error`, `isLoading` \- see [AsyncState](/utilities/react-hooks/useexec#asyncstate).

* `revalidate` is a method to manually call the function with the same arguments again.

* `mutate` is a method to wrap an asynchronous update and gives some control over how the `useFetch`'s data should be updated while the update is going through. By default, the data will be revalidated (eg. the function will be called again) after the update is done. See [Mutation and Optimistic Updates](/utilities/react-hooks/useexec#mutation-and-optimistic-updates) for more information.
...
```
export type ParseExecOutputHandler = (args: {
